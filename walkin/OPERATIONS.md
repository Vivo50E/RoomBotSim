# WALK-IN runtime operations

## Restart and recovery
At import/server boot, valid `jobs/<id>/world.json` plus `people.json` pairs are loaded into `server.JOBS`. They are ready for `/status`, episode listing, and `start_runtime`; simulation threads are **not** auto-started. Invalid/partial job directories are skipped, not modified. World and people updates use atomic replacement, so an interrupted write retains the previous valid version.
...
## Headless batches
Run without uvicorn from `walkin/`:

```sh
python tools/batch_run.py --job test --episodes 10 --task coffee_to_person --seed 7
```

The runner reloads an existing persisted job (or creates only the built-in `test` fixture), advances normal 100 Hz physics synchronously, writes episode JSONL, and exits. It never starts an HTTP server.
